    ...
    <div className="min-h-dvh bg-pizarra text-tiza">
      <header className="border-b border-tiza/15 sticky top-0 bg-pizarra/95 backdrop-blur z-20">
        <div className="mx-auto max-w-6xl px-4 h-16 flex items-center gap-4">
          <Link to={`/eventos/${id}`} className="text-sm font-medium text-tiza/60 hover:text-tiza">← Salir</Link>
          <div className="min-w-0">
            <p className="font-display text-lg truncate leading-tight">{e.nombre}</p>
          ...
          <label className="ml-auto flex items-center gap-2">
            ...
            <select
              ...
              className="bg-pizarra-2 border border-tiza/20 rounded-lg px-3 py-2 text-sm text-tiza">
              {e.equipo.map(m => (
      ...
      {aviso && (
        <div className="mx-auto max-w-6xl px-4 pt-4">
          <p className="bg-laurel/25 border border-laurel/50 text-tiza px-4 py-3 rounded-lg text-sm">{aviso}</p>
        </div>
      )}
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div>
          <div className="flex items-baseline justify-between mb-4">
            ...
            <button onClick={() => setPanelEgreso(v => !v)}
              className="text-sm text-tiza/60 hover:text-brillo underline underline-offset-4">
            ...
            {disponibles.map(p => {
              ...
              return (
                <button
                  ...
                  onClick={() => agregar(p)}
                  disabled={queda <= 0}
                  className="group text-left p-4 rounded-lg bg-pizarra-2 border border-tiza/12
                             hover:border-brillo active:translate-y-px transition
                ...
                >
                  ...
                  <span className="mt-auto pt-3 flex items-baseline justify-between">
                    <span className="cifra text-lg group-hover:text-brillo transition-colors">{money(p.precio)}</span>
                    <span className={`eyebrow ${queda < 15 ? 'text-alerta' : 'text-tiza/40'}`}>{num(queda)}</span>
                  </span>
                </button>
              )
            })}
        ...
        <aside className="lg:sticky lg:top-20 lg:self-start">
          <div className="bg-pizarra-2 border border-tiza/15 rounded-lg">
            <div className="px-4 py-3 border-b border-tiza/12 flex items-center justify-between">
              ...
              {carrito.length > 0 && (
                <button onClick={() => { setCarrito([]); setPagos([]) }}
                        className="text-sm text-tiza/45 hover:text-membrillo">Vaciar</button>
              )}
            </div>
            ...
            <div className="px-4 py-4 border-t border-tiza/15">
              ...
              <button
                ...
                className="btn w-full bg-brillo text-pizarra py-3.5 rounded-lg font-semibold disabled:opacity-30">
                Cobrar
              </button>
            </div>
          </div>
...
function BotonCantidad({ children, onClick, rotulo }) {
  return (
    <button onClick={onClick} aria-label={rotulo}
      className="w-9 h-9 rounded-lg border border-tiza/25 hover:border-brillo hover:text-brillo
...
function Cobro({ total, pagos, setPagos, falta, pagado, onCerrar, onConfirmar }) {
  ...
  return (
    <div className="fixed inset-0 z-30 bg-pizarra/92 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div className="w-full sm:max-w-md bg-pizarra-2 border border-tiza/20 rounded-t-lg sm:rounded-lg">
        <div className="px-5 py-4 border-b border-tiza/12 flex items-center justify-between">
          ...
          <button onClick={onCerrar} className="text-sm text-tiza/50 hover:text-tiza">Cerrar</button>
        </div>
        ...
        <div className="p-5 space-y-5">
          {pagos.length > 0 && (
            <ul className="space-y-2">
              {pagos.map((p, i) => (
                <li key={i} className="flex items-center justify-between text-sm bg-pizarra/60 px-3 py-2 rounded-lg">
                  ...
                  <span className="flex items-center gap-3">
                    ...
                    <button onClick={() => setPagos(ps => ps.filter((_, j) => j !== i))}
                            className="text-xs text-tiza/40 hover:text-membrillo">quitar</button>
                  </span>
                </li>
              ))}
            </ul>
              ...
              <div>
                <div className="flex items-baseline justify-between mb-2">
                  ...
                  <button onClick={() => setMonto(String(falta))}
                          className="text-sm font-medium text-brillo hover:underline underline-offset-4">
                    Todo: {money(falta)}
                  </button>
                </div>
                <input
                  ...
                  placeholder="0"
                  className="cifra w-full text-2xl px-4 py-3 rounded-lg bg-pizarra border border-tiza/25
                             focus:border-brillo outline-none text-tiza"
                />
                {vuelto > 0 && (
                  <p className="text-sm text-brillo mt-2">Vuelto: <span className="cifra">{money(vuelto)}</span></p>
                )}
              </div>

              <button onClick={() => sumar(Math.min(Number(monto), falta))}
                      disabled={!monto || Number(monto) <= 0}
                      className="btn w-full border border-tiza/30 py-3 rounded-lg text-sm hover:bg-pizarra disabled:opacity-30">
                Agregar este pago
              </button>
            </>
          )}
          ...
          <div className="pt-4 border-t border-tiza/12 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between font-semibold">
              ...
              <span className={`cifra ${falta > 0 ? 'text-alerta' : 'text-laurel'}`}>{money(Math.max(falta, 0))}</span>
            </div>
          </div>

          <button onClick={onConfirmar} disabled={falta > 0}
            className="btn w-full bg-laurel text-tiza py-3.5 rounded-lg font-semibold disabled:opacity-30">
            Confirmar la venta
          </button>
        </div>
      </div>
...
function PanelEgreso({ s, id, productos, onListo }) {
  ...
  return (
    <div className="mb-4 p-4 rounded-lg bg-pizarra-2 border border-brillo/40 grid gap-3 sm:grid-cols-[1fr_1fr_80px_auto]">
      <select value={productoId} onChange={e => setProductoId(e.target.value)}
        className="bg-pizarra border border-tiza/20 rounded-lg px-3 py-2.5 text-sm text-tiza">
      ...
      <input type="number" min="1" value={cantidad} onChange={e => setCantidad(Number(e.target.value))}
        className="cifra bg-pizarra border border-tiza/20 rounded-lg px-3 py-2.5 text-sm text-tiza" />
      <button
        onClick={() => {
        ...
        className="btn bg-brillo text-pizarra px-4 py-2.5 rounded-lg text-sm">
        Registrar
      </button>
      <input value={motivo} onChange={e => setMotivo(e.target.value)}
        placeholder="Motivo (opcional): se cayó una caja, muestra para la radio…"
        className="sm:col-span-4 bg-pizarra border border-tiza/20 rounded-lg px-3 py-2.5 text-sm text-tiza placeholder:text-tiza/35" />
    </div>
  )
}
